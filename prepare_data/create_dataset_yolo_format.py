...
for set_ in ['train', 'validation', 'test']:
    for dir_ in [os.path.join(DATA_OUT_DIR, set_),
        ...
        if os.path.exists(dir_):
            shutil.rmtree(dir_)
        dirs = [os.path.join('.', 'data', 'train'), os.path.join('.', 'data', 'val')]
        dirs = [os.path.join('.', 'data', 'train'), os.path.join('.', 'data', 'val')]

        for dir_ in dirs:
            # makedirs creates all missing folders in the path at once
            os.makedirs(dir_, exist_ok=True)
            print(f"Successfully verified or created: {dir_}")
